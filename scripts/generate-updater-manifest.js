...
async function generate() {
	...
	while (attempts < maxAttempts) {
		...
		const hasInstallers = assets.some(
			(a) => a.name.endsWith('.tar.gz') || a.name.endsWith('.msi') || a.name.endsWith('.AppImage')
		);
		...
		if (name.endsWith('.sig')) {
			...
			if (targetAsset) {
				...
				if (name.includes('aarch64.app.tar.gz')) {
					...
					name.includes('x64') &&
					(name.includes('.msi.zip') || name.includes('.nsis.zip') || name.includes('.zip'))
				) {
					platforms = ['windows-x86_64'];
				} else if (
					name.includes('amd64.deb') ||
					name.includes('x86_64.AppImage') ||
					name.includes('x86_64.tar.gz')
				) {
					platforms = ['linux-x86_64'];
				} else if (
					name.includes('aarch64.deb') ||
					name.includes('aarch64.AppImage') ||
					name.includes('aarch64.tar.gz')
				) {
