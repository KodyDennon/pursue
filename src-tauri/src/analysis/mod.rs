...
use crate::models::{AnalysisReport, RecordAsset, EntityHit};
...
impl AnalysisManager {
    ...
    pub async fn provision_models(&self, app: &tauri::AppHandle) -> Result<()> {
        ...
        for model in registry {
            ...
            let _ = self.models.ensure_model(app, &model.id, model.filename.as_deref().unwrap_or(&model.id), &model.repo_id).await;
        }
    ...
    async fn index_record_inner(&self, _app: &tauri::AppHandle, record_id: &str) -> Result<AnalysisReport> {
        ...
        let full_path = self.library.get_full_path(record.local_path.as_ref().unwrap());

        // 1. OCR (Foundation)
        let _ = _app.emit("analysis-progress", serde_json::json!({ "status": "extracting-foundation", "record_id": record_id }));
        ...
        if full_path.extension().and_then(|e| e.to_str()) == Some("pdf") {
            ...
            if let Ok(forensics) = self.indexer.pdf.extract_forensics(&full_path) {
                let _ = self.persistence.persist_forensics(record_id, &forensics).await;
            }
            // Images
            if let Ok(extracted) = self.indexer.pdf.extract_images(&full_path, &asset_dir).await {
        ...
        self.persistence.persist_entities(record_id, &entities).await?;
        let chunks_indexed = self.persistence.persist_chunks(record_id, &record.title, &text, &entities).await?;
        
        let redaction_score = self.indexer.ocr.analyze_redactions(&full_path).unwrap_or(0.0);
        sqlx::query("UPDATE records SET analysis_status = 'indexed', redaction_score = ? WHERE id = ?").bind(redaction_score).bind(record_id).execute(&self.db).await?;

        Ok(AnalysisReport { record_id: record_id.to_string(), status: "indexed".to_string(), ocr_text: text, entities, chunks_indexed, engine, intelligence_json: None, assets: Vec::new() })
    }
    ...
    async fn synthesize_intelligence_inner(&self, app: &tauri::AppHandle, record_id: &str) -> Result<AnalysisReport> {
        ...
        let specs = get_hardware_specs();
        let preferred_model = if specs.recommended_tier == IntelligenceTier::Elite { "gemma-4-e4b" } else { "gemma-4-e2b" };
        let model_path = self.models.models_dir().join(preferred_model);
        
        let image_paths: Vec<_> = assets.iter().filter(|a| a.asset_type == "image").map(|a| self.library.get_full_path(&a.local_path)).collect();

        let intelligence_json = self.extractor.extract_forensics(app, record_id, ExtractionConfig { preferred_model_path: Some(model_path), fallback_model_path: None, force_cpu: false }, &text, image_paths).await?;

        let intel_str = serde_json::to_string(&intelligence_json)?;
        sqlx::query("UPDATE records SET analysis_status = 'completed', intelligence_json = ? WHERE id = ?").bind(&intel_str).bind(record_id).execute(&self.db).await?;

        self.get_analysis(record_id).await?.ok_or_else(|| anyhow!("report missing"))
    }
    ...
    pub async fn get_analysis(&self, record_id: &str) -> Result<Option<AnalysisReport>> {
        ...
        Ok(Some(AnalysisReport { record_id: record_id.to_string(), status: row.get("analysis_status"), ocr_text: row.get::<Option<String>, _>("ocr_text").unwrap_or_default(), entities: Vec::new(), chunks_indexed: 0, engine: "stored".to_string(), intelligence_json: row.get("intelligence_json"), assets: Vec::new() }))
    }
}
...
pub fn extract_entities(text: &str) -> Vec<EntityHit> {
    ...
    let patterns = [
        ...
        (r"(?i)\b(orb|sphere|tic-tac|cylinder|disc|triangle)\b", "shape"),
        (r"(?i)\b(radar|ir|flir|sonar|visual|satellite)\b", "sensor"),
        (r"(?i)\b(hypersonic|transmedium|instantaneous acceleration)\b", "pattern"),
    ];
...
fn add_entity(e: &mut BTreeMap<(String, String), EntityHit>, n: &str, ty: &str, c: f64, s: &str) {
    ...
    e.entry((name.to_lowercase(), ty.to_string())).or_insert(EntityHit { 
