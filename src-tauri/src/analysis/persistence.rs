...
impl PersistenceManager {
    ...
    pub async fn persist_entities(&self, record_id: &str, entities: &[EntityHit]) -> Result<()> {
        ...
        for entity in entities {
            ...
            let eid: String = sqlx::query_scalar("SELECT id FROM entities WHERE name = ? AND entity_type = ?")
                .bind(&entity.name)
                .bind(&entity.entity_type)
                .fetch_one(&mut *tx)
                .await?;
                
            sqlx::query("INSERT INTO record_entities (record_id, entity_id, confidence) VALUES (?, ?, ?)")
                .bind(record_id)
                .bind(eid)
    ...
    pub async fn persist_chunks(&self, record_id: &str, title: &str, text: &str, entities: &[EntityHit]) -> Result<usize> {
        let mut tx = self.db.begin().await?;
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            ...
            sqlx::query("INSERT INTO vec_analysis_chunks (chunk_id, embedding) VALUES (?, ?)")
                .bind(&cid).bind(vblob).execute(&mut *tx).await?;
    ...
    pub async fn persist_forensics(&self, record_id: &str, discoveries: &[crate::analysis::pdf::ForensicDiscovery]) -> Result<()> {
        let mut tx = self.db.begin().await?;
