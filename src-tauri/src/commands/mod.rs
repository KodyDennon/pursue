...
pub async fn database_status(db: &SqlitePool, library: &LibraryManager) -> Result<DatabaseStatus> {
    ...
    let counts = sqlx::query(
        ...
        SELECT 
            ...
            (SELECT COUNT(*) FROM records WHERE analysis_status = 'completed') as analyzed_records,
            (SELECT COUNT(*) FROM records WHERE analysis_status = 'failed') as failed_analysis_records,
            (SELECT COUNT(*) FROM records WHERE analysis_status IS NULL OR analysis_status != 'completed') as unanalyzed_count,
