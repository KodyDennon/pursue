...
// Keep these compatibility helpers in Candle's namespace. CUDA 12.4 exposes
// __hmax_nan/__hmin_nan even when compiling for pre-SM80 devices, so defining
// functions with NVIDIA's names causes duplicate-definition failures on
// supported Turing GPUs such as SM75.
...
__device__ __forceinline__ __half atomicMaxf(__half* address, __half val) {
    ...
    do {
        ...
        hmax = __half_as_ushort(candle_hmax_nan(val, __ushort_as_half(hmax))); 
        old = atomicCAS(address_as_ui, assumed,
...
__device__ __forceinline__ __half atomicMinf(__half* address, __half val) {
    ...
    do {
        ...
        hmin = __half_as_ushort(candle_hmin_nan(val, __ushort_as_half(hmin))); 
        old = atomicCAS(address_as_ui, assumed,
