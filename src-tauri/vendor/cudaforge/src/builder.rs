...
impl KernelBuilder {
    ...
    pub fn build_lib<P: Into<PathBuf>>(&self, out_file: P) -> Result<()> {
        ...
        if compile_jobs.is_empty() && out_file.exists() {
            eprintln!("All library kernels up-to-date, skipping compilation");
            return Ok(());
        }

        eprintln!(
            "Compiling {} of {} kernels",
            compile_jobs.len(),
            kernel_files.len()
        );
    ...
    pub fn build_ptx(&self) -> Result<PtxOutput> {
        ...
        if compile_jobs.is_empty() {
            eprintln!("All PTX kernels up-to-date, skipping compilation");
            return Ok(PtxOutput {
                paths: kernel_files,
