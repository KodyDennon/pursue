...
fn extract_prebuilt_lib_names(
    ...
    for dir in [
        ...
        // Source builds keep the generated build-info functions used by
        // llama-common in this companion archive rather than installing it.
        prebuilt_root.join("build").join("common"),
    ] {
        if !dir.exists() {
            continue;
        }
...
fn main() {
    ...
    // Respect an explicit parallelism limit. Bound Windows NVCC fan-out while
    // still using available machine resources; callers can override this.
    if env::var_os("CMAKE_BUILD_PARALLEL_LEVEL").is_none() {
        let parallelism = if target.contains("windows") {
            std::thread::available_parallelism().unwrap().get().min(12)
        } else {
            std::thread::available_parallelism().unwrap().get()
        };
        // TODO: Audit that the environment access only happens in single-threaded code.
        unsafe { env::set_var("CMAKE_BUILD_PARALLEL_LEVEL", parallelism.to_string()) };
    }
    ...
    if let Some(prebuilt_dir) = resolve_prebuilt_directory(&target, build_shared_libs) {
        if prebuilt_dir.exists() {
            ...
            eprintln!(
                "Using prebuilt llama libs from {}",
            ...
            for p in [
                ...
                prebuilt_dir.join("build").join("common"),
            ] {
                if p.exists() {
    ...
    let parallel = env::var("CMAKE_BUILD_PARALLEL_LEVEL")
        .ok()
    ...
    if cfg!(feature = "cuda") && !cfg!(target_os = "macos") {
        ...
        if target.contains("windows") {
            // Generated kernels intentionally cast large sentinels to float.
            // Suppress only NVIDIA diagnostic 221; all other diagnostics stay visible.
            config.define("CMAKE_CUDA_FLAGS", "--diag-suppress=221");
        }
    }
