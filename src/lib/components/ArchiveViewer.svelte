<script lang="ts">
	...
	const imageAssets = $derived(
		(analysis?.assets ?? []).filter((asset) => asset.asset_type === 'image')
	);
...
<article class="archive-viewer">
	<header class="viewer-head">
		<div class="button-row">
			...
			<button onclick={revealLocal} disabled={!record.local_path || busy === 'open-path'}
		...
		<h2>{record.title}</h2>
		<p>
			{record.agency || 'Unknown agency'} · {record.release_date || 'undated'} · {record.incident_location ||
				'no location'}
		</p>
		...
		<div class="viewer-actions">
			<button
				class="primary"
				onclick={download}
				disabled={!!record.local_path || !record.document_url || busy === 'download'}
			>
			...
			<button
				class="primary"
				onclick={analyze}
				disabled={!record.local_path || busy === 'analysis'}
			>
				{busy === 'analysis'
					? 'Analyzing...'
		...
		{#if activeTab === 'overview'}
			<section class="viewer-section">
				...
				<dl class="record-meta">
					...
					<div>
						<dt>Stable Key</dt>
						<dd>{record.stable_key || 'not recorded'}</dd>
					</div>
					<div>
						<dt>Source Type</dt>
					...
					<div>
						<dt>File Type</dt>
						<dd>{record.file_type || 'unknown'}</dd>
					</div>
					<div>
						<dt>Artifact Size</dt>
						<dd>{formatBytes(record.artifact_size)}</dd>
					</div>
					<div>
						<dt>Artifact SHA-256</dt>
						<dd>{short(record.artifact_sha256)}</dd>
					</div>
					<div>
						<dt>Content Hash</dt>
						<dd>{short(record.content_hash)}</dd>
					</div>
					<div>
						<dt>Analysis Status</dt>
						<dd
							class:status-completed={record.analysis_status === 'completed'}
							class:status-indexed={record.analysis_status === 'indexed'}
						>
		...
		{:else if activeTab === 'analysis'}
			<section class="viewer-section">
				...
				<dl class="record-meta">
					<div>
						<dt>Status</dt>
						<dd>{analysis?.status || record.analysis_status || 'pending'}</dd>
					</div>
					<div>
						<dt>Engine</dt>
						<dd>{analysis?.engine || 'not run'}</dd>
					</div>
					<div>
						<dt>Chunks Indexed</dt>
						<dd>{analysis?.chunks_indexed ?? 0}</dd>
					</div>
					<div>
						<dt>Redaction Score</dt>
						<dd>
							{record.redaction_score == null
								? 'not analyzed'
								: `${(record.redaction_score * 100).toFixed(2)}%`}
						</dd>
					</div>
				</dl>
			</section>
			...
			<section class="viewer-section">
				...
				{#if analysis?.entities.length}
					<div class="entity-grid">
						{#each analysis.entities as entity (entity.id)}
							<span class="entity-pill"
								>{entity.entity_type}: {entity.name} ({entity.confidence.toFixed(2)})</span
							>
						{/each}
					</div>
				{:else}
			...
			<section class="viewer-section">
				...
				{#if analysis?.intelligence_json || record.intelligence_json}
					<pre class="ocr-text">{formatJson(
							analysis?.intelligence_json || record.intelligence_json
						)}</pre>
				{:else}
			...
			<section class="viewer-section">
				...
				<p>
					{selectedCase
						...
						: 'Create or select a case from the Cases view.'}
				</p>
				<textarea bind:value={noteBody} rows="5" placeholder="Enter case analysis observations..."
				></textarea>
				<div class="button-row">
					<button onclick={addToCase} disabled={!selectedCaseId || busy === 'case-add'}
						>Add Record</button
					>
					<button
						onclick={addNote}
						disabled={!selectedCaseId || !noteBody.trim() || busy === 'case-note'}>Add Note</button
					>
					<button
