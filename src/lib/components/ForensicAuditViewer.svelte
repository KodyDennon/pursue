<script lang="ts">
  ...
  let { recordId, forensics = [], images = [] } = $props<{
    recordId: string;
      ...
      <div class="tactical-hud">
        ...
        {#if currentImage}
          <div class="document-stage" class:xray-active={showXRay}>
            ...
            {#if showXRay}
              <svg class="forensic-overlay" viewBox="0 0 1000 1414" preserveAspectRatio="xMidYMid meet">
                {#each activeForensics as item}
                  ...
                  {#if box}
                    ...
                    >
                      ...
                      />
                      <line x1={box[0]} y1={box[1]} x2={box[0]-20} y2={box[1]-20} class="connector" />
                      <text x={box[0]-25} y={box[1]-25} class="node-label">{item.layer_type.replace('_', ' ').toUpperCase()}</text>
                    </g>
                  {/if}
                {/each}
              </svg>
...
<style>
  ...
  .node-rect.hidden { stroke: #ffb300; fill: rgba(255, 179, 0, 0.1); }
  .node-rect.improper { stroke: #ff3c00; fill: rgba(255, 60, 0, 0.1); }
  ...
  .connector { stroke: rgba(255,255,255,0.2); stroke-width: 1; opacity: 0; transition: opacity 0.2s; }
  .node-label { fill: #fff; font-size: 12px; font-weight: 900; opacity: 0; transition: opacity 0.2s; font-family: var(--font-mono); }

  .discovery-node.hovered .connector, 
  .discovery-node.hovered .node-label { opacity: 1; }
  ...
  .xray-active .scan-line { opacity: 0.5; }
  ...
  .side-header h2 { font-size: 18px; margin: 8px 0; color: #fff; }
  .prefix { font-size: 9px; font-weight: 900; color: var(--text-tertiary); letter-spacing: 0.2em; }

  .discovery-stack { display: flex; flex-direction: column; gap: 16px; }
  ...
  .d-type-tag { font-size: 9px; font-weight: 900; color: var(--accent-primary); margin-bottom: 12px; letter-spacing: 0.1em; }
  .d-snippet { font-family: var(--font-mono); font-size: 12px; color: #fff; line-height: 1.5; margin: 0 0 12px 0; }
  .d-meta { display: flex; justify-content: space-between; font-size: 10px; color: var(--text-tertiary); }
  ...
  .empty-stage { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 24px; color: var(--text-tertiary); }
