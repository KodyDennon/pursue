<script lang="ts">
  import { X, ExternalLink, Download, Maximize2, Minimize2, ZoomIn, ZoomOut, RotateCw } from "lucide-svelte";
  ...
  const isImage = $derived(["jpg", "jpeg", "png", "webp", "gif", "bmp", "tif", "tiff"].includes(fileType));
        ...
        <div class="media-actions">
          {#if isImage}
            <div class="tool-group">
              <button onclick={() => zoom = Math.max(0.5, zoom - 0.2)} title="Zoom Out"><ZoomOut size={18} /></button>
              <span class="zoom-pct">{Math.round(zoom * 100)}%</span>
              <button onclick={() => zoom = Math.min(5, zoom + 0.2)} title="Zoom In"><ZoomIn size={18} /></button>
              <button onclick={() => rotation = (rotation + 90) % 360} title="Rotate"><RotateCw size={18} /></button>
            </div>
          {/if}
